# GARCH Model Fitting for Financial Time Series Analysis (Manual Engine)
# This script implements and evaluates various GARCH-family models using the manual engine
# to ensure like-for-like comparison with NF-GARCH models
...
plot_returns_and_save <- function(returns_list, prefix) {
  ...
  dir_path <- file.path("results/manual_results/plots", paste0("histograms_", prefix))
...
ts_cross_validate <- function(returns, model_type, dist_type = "sstd", submodel = NULL, 
  ...
  for (start_idx in seq(1, n - window_size - forecast_horizon, by = step_size)) {
    ...
    if (!is.null(fit) && engine_converged(fit)) {
      # Generate forecasts
      forecast <- tryCatch({
        engine_forecast(fit, h = forecast_horizon)
      }, error = function(e) {
...
# Train the GARCH Models using Chrono split and TS CV Split with Manual Engine
...
for (config_name in names(model_configs)) 
{
  ...
  equity_chrono_split_fit <- lapply(names(equity_train_returns), function(asset) {
    tryCatch({
      engine_fit(
        model = cfg$model, 
        returns = equity_train_returns[[asset]], 
  ...
  fx_chrono_split_fit <- lapply(names(fx_train_returns), function(asset) {
    tryCatch({
      engine_fit(
        model = cfg$model, 
        returns = fx_train_returns[[asset]], 
        dist = cfg$distribution, 
        submodel = cfg$submodel, 
        engine = "manual"
      )
...
run_all_cv_models <- function(returns_list, model_configs, window_size = 500, forecast_horizon = 40) {
  ...
  for (model_name in names(model_configs)) {
    ...
    message("⚙️ Running CV for model: ", model_name, " (manual engine)")
...
rank_models <- function(results_df, label = NULL) 
{
  results_df %>%
    ...
    arrange(Avg_MSE) %>%
    mutate(Source = label, Engine = "manual")
}  
...
for (key in names(Fitted_Chrono_Split_models)) 
{
  ...
  comparison <- setNames(
    Map(function(fit, forecast, ret) {
      if (!is.null(fit) && !is.null(forecast)) {
...
{
  dir_path <- file.path("results/manual_results/plots", paste0("volatility_", model_name, "_", asset_type))
  dir.create(dir_path, recursive = TRUE, showWarnings = FALSE)
  
  for (asset in names(forecast_list)) {
    if (!is.null(forecast_list[[asset]])) {
      sigma_vals <- forecast_list[[asset]]$sigma
      
      png(file.path(dir_path, paste0(asset, "_volatility.png")), width = 800, height = 600)
      plot(sigma_vals, main = paste("Volatility Forecast:", asset, "-", model_name, "(Manual Engine)"), col = "blue", ylab = "Volatility")
      dev.off()
    }
  }
}
...
if (!dir.exists("results/manual_results/consolidated")) {
  dir.create("results/manual_results/consolidated", recursive = TRUE, showWarnings = FALSE)
}
